...
export default function ProductsPage() {
  ...
  const deleteViaApi = async (productIds: string[]) => {
  ...
  const handleDeleteProduct = async (productId: string) => {
    if (!confirm('Are you sure you want to delete this product? This will also remove its images, variants, reviews, and cart/wishlist entries.')) return;

    try {
      await deleteViaApi([productId]);
  ...
  const handleBulkDelete = async () => {
    if (!confirm(`Are you sure you want to delete ${selectedProducts.length} product(s)? This will also remove their images, variants, reviews, and cart/wishlist entries.`)) return;

    try {
      await deleteViaApi(selectedProducts);
