...
function OrderTrackingContent() {
  ...
  const fetchOrder = useCallback(async (orderNum: string, lookupEmail?: string) => {
    setLoading(true);
    setError('');
    ...
    try {
      ...
      if (session?.user) {
        ...
        if (!fetchError && data) orderData = data;
      }

      if (!orderData) {
        const emailForLookup = lookupEmail || email;
        if (!emailForLookup) {
          setError('Please enter your email address to look up your order.');
          setIsTracking(false);
          setLoading(false);
          return;
        }
        ...
        const res = await fetch('/api/orders/lookup', {
          ...
          body: JSON.stringify({ order_number: orderNum, email: emailForLookup }),
        });
  ...
  useEffect(() => {
    if (urlOrderNumber) {
      ...
      fetchOrder(urlOrderNumber, storedEmail || undefined);
    }
          ...
          <div className="space-y-4">
            {order.order_items?.map((item: any) => (
              <div key={item.id} className="flex items-center space-x-4 p-4 bg-gray-50 rounded-lg">
                <div className="w-20 h-20 bg-gray-200 rounded-lg overflow-hidden flex-shrink-0 border border-gray-200 relative">
