...
export default function PaymentPage() {
  ...
  const [paymentMethod, setPaymentMethod] = useState<'moolre'>('moolre');
  ...
  useEffect(() => {
    ...
    async function fetchOrder() {
      try {
        ...
        if (storedEmail) {
          await lookupWithEmail(storedEmail);
        } else {
          setNeedsEmail(true);
          setLoading(false);
        }
  ...
  const lookupWithEmail = async (lookupEmail: string) => {
    setLoading(true);
    setError(null);
    try {
      const res = await fetch('/api/orders/lookup', {
        ...
        body: JSON.stringify({ order_number: orderId, email: lookupEmail }),
      });
      const result = await res.json();

      if (!result.success) {
        setError('Order not found. Please check your link and email address.');
        setNeedsEmail(true);
        setLoading(false);
        return;
      }
        ...
        <div className="max-w-md w-full">
          <div className="text-center mb-8">
            <div className="w-16 h-16 mx-auto mb-4 bg-gray-100 rounded-full flex items-center justify-center">
              <i className="ri-mail-check-line text-3xl text-gray-600"></i>
            </div>
            <h1 className="text-2xl font-bold text-gray-900 mb-2">Verify Your Email</h1>
            <p className="text-gray-600">Enter the email address used for this order to continue.</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm p-6">
            <form onSubmit={(e) => { e.preventDefault(); if (emailInput) lookupWithEmail(emailInput); }} className="space-y-4">
              <input
                type="email"
                value={emailInput}
              ...
              <button
                ...
                className="w-full bg-gray-700 hover:bg-gray-800 text-white py-3 rounded-lg font-semibold transition-colors disabled:opacity-50"
              >
    ...
    <main className="min-h-screen bg-gray-50 py-12 px-4">
      <div className="max-w-lg mx-auto">
        ...
        <div className="mb-6 bg-white rounded-xl shadow-sm p-6">
          ...
          <p className="text-sm text-gray-600 mb-4">Pay securely via Mobile Money (MTN, Vodafone, AirtelTigo).</p>
          <div className="flex items-center gap-4 p-4 rounded-xl border-2 border-gray-900 bg-gray-50">
            ...
            <div>
              ...
              <span className="text-xs text-gray-600">MTN MoMo, Vodafone Cash, AirtelTigo Money</span>
            </div>
          </div>
        </div>
