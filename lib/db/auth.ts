...
/** Service-role style user creation (seed scripts / admin bootstrap). */
export async function adminCreateUser(opts: {
  ...
  if (existing.rows[0]) {
    const user = await getUserById(existing.rows[0].id);
    return { user, error: null };
  }
  ...
  await query(
    `INSERT INTO auth.users (
       ...
       $1, '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
       $2, $3, ${opts.email_confirm !== false ? "now()" : "NULL"},
...
export async function adminListUsers(opts?: {
  ...
  const perPage = Math.min(opts?.perPage ?? 1000, 1000);
  ...
  try {
    ...
    for (const row of rows) {
      users.push(rowToUser(row, await loadAppRole(row.id)));
    }
