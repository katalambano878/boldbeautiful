...
self.addEventListener('fetch', (event) => {
  ...
  // HTML / navigations / Next data: network only Ã¢â€ â€™ offline shell
  if (
...
self.addEventListener('push', (event) => {
  ...
  const options = {
    body: data.body || 'New update from Bold Beautiful',
  ...
  event.waitUntil(
    self.registration.showNotification(data.title || 'Bold Beautiful', options)
  );
});
